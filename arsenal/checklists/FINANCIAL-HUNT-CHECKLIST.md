...
This extends DEFI-FULLSTACK-CHECKLIST with finance-specific patterns.
...
|-----------|------|-----------|--------|---------------|
| OB-001 | Consent scope escalation | TPP requests consent for accounts → gets access to all accounts, not just consented | Unauthorized data access | Art. 67(2)(d) |
| OB-002 | Consent not expiring | Consent should expire after 90 days (regulatory requirement) → doesn't | Persistent unauthorized access | RTS Art. 10(2) |
| OB-003 | SCA downgrade | Strong Customer Authentication bypassed or downgraded to single factor | Auth bypass on payments | RTS Art. 4 |
...
| OB-005 | IBAN validation bypass | Payee IBAN not validated → payment to wrong account accepted | Misdirected payments | PSD2 Art. 88 |
| OB-006 | Rate limit bypass on AIS | Account information endpoint not rate-limited → full account scraping | Mass data exfiltration | RTS Art. 36 |
| OB-007 | Certificate validation bypass | eIDAS/QWAC certificate not validated → unauthorized TPP access | Impersonation of regulated TPP | RTS Art. 34 |
| OB-008 | Fallback interface abuse | Screen scraping fallback activated unnecessarily → weaker security | Auth credential exposure | RTS Art. 33 |
| OB-009 | Payment status manipulation | POST payment → poll status → status can be modified or replayed | Double payment, payment cancellation | PSD2 Art. 80 |
| OB-010 | Cross-TPP data leakage | Data consented to TPP-A accessible by TPP-B via API flaw | Privacy violation, data breach | GDPR Art. 5(1)(b) |
...
# Berlin Group: https://xs2a.{bank}/sandbox/
# UK OB: https://ob.{bank}/open-banking/v3.1/sandbox/
# Generic: https://api.{bank}/sandbox/, https://developer.{bank}/
...
# Example: add attacker as "trusted beneficiary" via IDOR, then transfer without SCA
...
|----------|------|----------|-------|----------|
| **Mojaloop** | Open-source payment hub | JS/TS | Financial inclusion (Bill & Melinda Gates Foundation) | P2 |
...
|---------|-----------|--------|-----------|
| XXE injection | XML external entity in ISO 20022 message → SSRF/file read on parser | Data exfiltration from bank servers | Inject XXE payloads in text fields (RemittanceInformation, etc.) |
| Billion laughs | Entity expansion bomb in message → memory exhaustion | DoS on payment processing | Nested entity definitions |
| Schema confusion | Message claims to be pacs.008 but contains elements from camt.054 | Parser confusion, misrouted payment | Cross-schema element injection |
| Amount overflow | `<IntrBkSttlmAmt>` with extremely large value → integer overflow in parser | Incorrect payment amount | Boundary values: MAX_DECIMAL, negative, huge exponent |
| IBAN injection | IBAN field contains XML/SQL special characters → injection in downstream | Injection in banking systems | Special chars in IBAN/BIC fields |
| Encoding confusion | UTF-8 BOM, overlong UTF-8, mixed encodings in single message | Parser bypass, data corruption | Encoding edge cases |
| Structured address exploitation | Structured address fields (PSD3 mandatory) with overlong/malformed data | Buffer overflow in legacy systems | Field length boundary testing |
...
Every payment processor uses webhooks to notify merchants of events (payment completed, refund initiated, dispute opened). Webhook verification is a critical trust boundary.
...
|--------|-----------|--------|
| Unsigned webhook injection | Send fake webhook without signature → merchant processes it | Free goods, fake refunds |
| Webhook replay | Capture valid webhook → replay later | Double-processing of payment |
| Webhook SSRF (PayPal) | `PAYPAL-CERT-URL` header → merchant fetches URL → SSRF | Internal network access |
| Timing attack on HMAC | Non-constant-time comparison of webhook signature | Signature forgery via timing oracle |
| Webhook event confusion | Send `payment_intent.succeeded` for an unpaid order | Free goods |
| Webhook idempotency bypass | Same event ID sent twice → both processed | Double-processing |

### 3.4 IBAN/BIC Validation Library Bugs

```
IBAN validation is implemented by dozens of libraries, and each handles
edge cases differently. A validation bypass = misdirected payments.
...
1. **DeFi bounties** (Immunefi, direct) — ✅ producing results
