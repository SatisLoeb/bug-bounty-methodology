...
`upshift2` reads these as references and invokes them at the named phase. It does not duplicate their work and does not auto-orchestrate them mid-hunt. **The Phase-6 disclosure channels (/disclose, /immunefi-submit, /security-disclosure) are OPERATOR-GATED (U2-1 fix): the operator explicitly authorizes each send; never auto-spawn a disclosure — it is outward-facing and hard to reverse (firmaudit no-auto-orchestration rule).**

> **Tool-existence guard (U2-2 fix).** The companion files (SEAM-THESIS/HUNT-METHODOLOGY/IMMORTAL-MODE/ANTI-INFLATION/...), the `vectors/*` packs, and the gates below are HARDCODED paths. Before reading/running one: `ls <path> 2>/dev/null` — if MISSING (fresh install / machine-2 not synced / moved), NOTE "resource X unavailable, phase Y degraded" and CONTINUE; never crash, never silently skip the phase.
