...
2. **Anti-AI signal** — a real terminal screenshot (with prompt, syntax highlighting, line wrap, occasional typo) is one of the strongest human-authenticity signals available. Anti-AI classifiers on HackerOne, Cantina, and HackenProof have no way to flag a `.png` of a working terminal.
   ...
   > **SS-1 caveat (darkside-on-screenshot 2026-06-23): the 'can't flag a .png' assumption is NOT permanent.** The robust value of a screenshot is purposes 1 + 3 (independent verification of mainnet state + effort signal) — those hold regardless of AI detection. The anti-AI-evasion value (purpose 2) rests on an outdated premise: 2026 multimodal triage CAN OCR the image and run the same text-classifier on the extracted output, and a CONSISTENT terminal prompt/theme/font/window across many submissions is itself a cross-report fingerprint (the chill C-1 problem, in pixels). So: lean on verification + effort as the load-bearing value; treat anti-AI-evasion as a bonus, not a guarantee; and vary the terminal surface (prompt, theme) across submissions so the screenshots don't become their own signature.
...
- **immunefi-submit**: if the target is Immunefi (boycott LIFTED 2026-06-01 — now a live channel like HackerOne/Cantina; SS-2 fix), the screenshot guide format adapts to Immunefi's submission UI
