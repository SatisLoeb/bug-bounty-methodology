...
name: upshift
description: >-
  End-to-end methodology for engaging DeFi protocols with off-chain orchestration (delta-neutral stables, RWA tokenized accounts, yield aggregators with keepers, LRTs, perp DEXes, vault curators). Codifies the Upshift Finance engagement that produced 48 findings, ack in 24h, and patches in 30h. Use when target has the profile: backend that signs on-chain transactions in response to off-chain events, executor/operator wallet pattern, contracts audited by a known firm but API/infra/frontend treated as a normal web project. Coordinates /gravedigger and /mrrobbot as reference cadence; /disclose stays OPERATOR-GATED (never auto-spawned -- a disclosure is an outward-facing, hard-to-reverse action). Includes immortal-mode discipline and live re-verification monitor for active disclosures.
---
...
> **Tool-existence guard (U-2 fix).** The companion files + external playbooks + gates below are HARDCODED paths. Before reading/running one: `ls <path> 2>/dev/null` — if MISSING (fresh install / machine-2 not synced / moved), NOTE "resource X unavailable, phase Y degraded" and CONTINUE; never crash, never silently skip a phase.
...
> **Reconciliation with firmaudit (U-1 fix).** When this cadence runs UNDER firmaudit (Path B), firmaudit's no-auto-orchestration rule GOVERNS — read upshift/gravedigger/mrrobbot as REFERENCE, the operator hand-orchestrates between phases. Standalone `/upshift` (the operator explicitly invoked it) coordinates the cadence below, but STILL gates /disclose to an explicit operator OK and NEVER auto-spawns the disclosure flow. No skill fires another from a passive signal.
...
| `/disclose` | Phase 5 -- per-finding report formatting. **OPERATOR-GATED: never auto-spawned; the operator explicitly authorizes each disclosure (firmaudit no-auto-orchestration rule — a disclosure is outward-facing).** |
...
**Pass 3b: Pierce every gate — GATED IS NEVER A VERDICT (GATING, 2026-06-17 operator directive).** A gated surface ("behind auth / 401 / requires-login → blocked → next") is an un-executed hypothesis dressed as a conclusion. A gate names a control's EXISTENCE, never its STRENGTH; a 401 is a sign something valuable is HERE — the seam where two auth regimes meet is exactly an upshift seam (on-chain-vs-off-chain-operator, HMAC-API-vs-session, REST-vs-WS). On EVERY gated surface run the 4 bypass angles before "blocked": **(A) AUTHN≠AUTHZ** — the gate proves a valid token is needed; does the controller verify THIS token OWNS THIS resource? a resource-id with no ownership bind = IDOR/BFLA (a global pre-routing authn filter that 401s even nonexistent paths is solid against unauth probing but moves ALL the value to the post-filter authz it does NOT test). **(B) UNGUARDED SIBLING** — v1-vs-v2 drift, REST-vs-WS-vs-GraphQL, the operator/keeper signing path vs the user path, web-vs-mobile, staging mirror. **(C) GATE SATISFIABLE** — token before 2FA completes (M-H1-007), destructive op takes a standard credential while a read needs sudo (M-H1-002 inversion), replayable sig. **(D) DIFFERENTIAL LEAK** — 401-vs-403-vs-404-vs-timing leaks other users' resource state. Piercing has TWO honest outcomes: find the window, OR prove the gate holds against THIS angle → value moves to the NEXT angle, never "done." A recon-only probe that can't reach the authz layer scopes the SUSPENDED harness, it doesn't prove a bypass. See `feedback_gated_is_not_a_verdict_pierce_the_gate.md`.
...
## Activation suggestions (SUGGEST, never auto-fire -- U-1 fix, 2026-06-23)

Beyond explicit `/upshift` invocation, when the signals below appear, **SUGGEST** to the operator "this looks upshift-class -- run `/upshift --triage`?" and WAIT for explicit confirmation. NEVER auto-fire the engagement yourself: auto-firing a multi-skill flow (gravedigger + mrrobbot + the /disclose disclosure) from a passive signal violates firmaudit's NON-NEGOTIABLE no-auto-orchestration rule and risks an unauthorized outward-facing disclosure cascade. Signals:
...
On EXPLICIT operator confirmation, route to `/upshift --triage <target>` first to confirm the profile. (U-1 fix: SUGGEST, never auto-fire; the operator stays in the loop.)
