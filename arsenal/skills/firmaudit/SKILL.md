...
- Saturation CLOSED **but** a non-saturated surface IS accessible → **GO, but the plan MUST target that surface, not the audited core.** Route to the matching cadence (Web2-on-SC → `WEB2-ON-SC-PROGRAMS-PLAYBOOK.md`; off-chain → Path B upshift; crypto-lib → `CRYPTO-LIB-HUNT-PLAYBOOK.md`; infra-adjacent → `INFRA-ADJACENT-TO-SC-PLAYBOOK.md`). The deep plan hunts the un-audited zone; it does NOT spend the slot on incomplete-fix of the audited core.
- Saturation OPEN (fresh code, <3 audits, or a real post-audit delta) → proceed to T7 normally.
