...
### 4.1b Mirror Invariant Audit (MANDATORY for in/out programs — CLAUDE.md rule #41)

For Solana programs with paired state-changing instructions (deposit/withdraw, stake/unstake, mint/burn, lock/unlock, bridge in/out), the mirror rule applies identically to EVM:

```bash
# Enumerate instruction pairs
grep -nE "pub fn (deposit|withdraw|stake|unstake|mint|burn|lock|unlock|wrap|unwrap|open|close|fund|release|swap_in|swap_out|bridge_in|bridge_out|claim|refund)" --include="*.rs" programs/
```

For each pair, build the V_in / V_out validation comparison table:

| Validation | V_in | V_out | Asymmetry? | Design reason? |
|---|---|---|---|---|
| `has_one` constraint | ✓/✗ | ✓/✗ | | |
| `Signer<'info>` on authority | ✓/✗ | ✓/✗ | | |
| PDA seed bump canonical | ✓/✗ | ✓/✗ | | |
| Amount bounds (MIN_DEPOSIT, MAX_WITHDRAW) | ✓/✗ | ✓/✗ | | |
| Token account owner check | ✓/✗ | ✓/✗ | | |
| Transfer fee reconciliation (Token-2022) | ✓/✗ | ✓/✗ | | |
| Reentrancy guard (CPI ordering) | ✓/✗ | ✓/✗ | | |

Asymmetries without articulable design reason → finding-candidate → Kill Gate. See §3.6 in `CRITICAL-HUNT-CHECKLIST.md` and `feedback_mirror_invariant_audit.md` for the full methodology.

**Solana-specific mirror pitfalls:**
- `process_deposit` pulls via `token::transfer` with fee reconciliation; `process_withdraw` pushes without fee awareness → shortfall for Token-2022 fee tokens
- `open_position` sets `state.initialized = true`; `close_position` does NOT check `state.initialized` → uninitialized close call accepted
- `stake` enforces `stake_amount >= MIN_STAKE`; `unstake` has no upper bound on the amount decoded from user input → overflow into negative
- `mint` requires `mint_authority` signer; `burn` does not require any authority → anyone can burn others' tokens if account ownership check is also missing
