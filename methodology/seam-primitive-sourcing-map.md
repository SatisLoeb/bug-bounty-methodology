...
**P7 — Encodage non-canonique → collision de digest / decode-boundary** · axe représentation cross-lang · *cadavre-primitive: A6 (Wormhole) · 1er cadavre-de-SOURCING: Lombard Solana 2026-09-17 (négatif → produit un écran)*
...
- **ÉCRAN pré-GO (le cadavre Lombard, falsifiable) : le chemin de CONSOMMATION re-sérialise-t-il + hash-check contre le hash signé ?** Si oui → **P7 MORT sur cette cible, ne GO pas** : le décodeur ne peut pas diverger des bytes signés sans casser le re-hash (Lombard `bridge::gmp_receive` exige `sha256(message.to_session_payload())==payload_hash` → offset/padding/trailing non-canoniques rejetés wholesale ; `abi_utils` overflow-guard chaque mot ; leftover-check sur le native mint). GO seulement si le commit signe un digest SANS re-sérialisation canonique au consume (ex. Wormhole VAA lu par offset-arith sans re-encode).
- impact : deux messages même digest signé / ré-interprétation de payload · **mur MOYEN** · où : bridges multi-langage (Wormhole guardian Go + Solidity + receivers Rust/Solana). **CAVEAT fraîcheur-FAUSSE (cadavre Lombard) : « code non-EVM moins-fouillé » est un faux signal de surface** — une équipe multi-stack disciplinée PORTE ses gardes et POSSÈDE la frontière (le crate `abi_utils` partagé + le pattern round-trip EST le propriétaire du boundary). L'axe = « la frontière a-t-elle un propriétaire qui la défend ? » pas « quel langage est plus récent ». GO le non-EVM seulement là où le décodeur est un PORT indépendant sans crate-frontière partagé — pas un re-port discipliné. Voir [[seam-density-is-the-surface-selection-axis]].
