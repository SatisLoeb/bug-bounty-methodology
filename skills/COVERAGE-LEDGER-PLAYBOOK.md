# COVERAGE-LEDGER PLAYBOOK — enumerate every in-scope file BEFORE any fortress/null verdict

**Shared mandatory gate. Invoked by `firmaudit` (before Phase T verdict) and `darkside` (gate 0.5.6, before any fortress/null). One source of truth — both skills reference this file, neither duplicates it.**

## The defect this fixes (Mezo, 2026-06-11 — the operator's statistical catch)

Seam-first hunting (firmaudit Phase S) and dup-aware prioritization are a STRENGTH — they put immortal depth where the bug is probable. But they have one structural blind spot: **they optimize for "find the bug where it's likely," not for "prove you looked everywhere."** The result: you deep-beast 8 high-signal surfaces, prove them sound, and write **"fortress"** — while N other in-scope files were never enumerated, never cocked, never even listed. Nothing in the skills forced you to write down the full file list and confront the gap.

Concretely on Mezo: the prior engagement named 8 surfaces (PCV/bridge/EVM/oracle/EIP-712/refinance/liquidation) and concluded fortress. A `find *.sol` showed musd has **20 contracts; 8 were never a named surface** (SortedTroves 500 LOC, HintHelpers 249, GovernableVariables 139, the 4 pools, MUSD token, TokenDeployer = ~1631 uncovered LOC). They got skipped by two reflexes, each individually reasonable and collectively a coverage hole:
...
## THE LEDGER — mechanical, runs before any fortress/null verdict
...
- **`UNCOVERED`** — never looked at. **Any `UNCOVERED` row makes a fortress verdict ILLEGAL.** Either cover it (Step 3) or convert it to a justified `SKIPPED`.
...
### Step 4 — the FORTRESS-VERDICT GATE
**A "fortress" / "null" / "NO-GO" verdict on the TARGET is illegal while any row is `UNCOVERED`, or any `SKIPPED` row's reason is on the not-allowed list.** Before writing the verdict, paste the ledger: every file is `COVERED+artifact`, `COVERED+seam-null`, or `SKIPPED+allowed-reason`. If you can't, you have a coverage hole — close it first. The verdict sentence must be able to say: "I enumerated N in-scope files; M covered with executed artifacts, K skipped for [allowed reasons]; 0 uncovered."
...
## The one-line rule
**Seam-thesis says where to look FIRST and deepest; the coverage-ledger proves nothing was forgotten LAST. A fortress verdict requires BOTH: the deep surfaces proven sound AND a complete ledger with zero uncovered rows.** Run this before every fortress/null/NO-GO, on every audited target.
