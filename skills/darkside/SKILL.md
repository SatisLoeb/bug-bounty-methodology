...
> **No catalogue hook by design (F-3 fix, 2026-06-23).** `corpus-query` is a CATALOGUE tool; this skill is THIEF-mode discovery — Door C explicitly does NOT fan-out because pattern-matching SUPPRESSES the surprise a singularity needs. Run the corpus pattern-bank in mrrobbot / firmaudit-Pass-2 (completeness), NEVER at the entry of darkside.
...
- **Saturation score** (run at intake): `audit_count ≥ 2` is a heavy **signal** (especially if one is
  ...
  + clean payout history → **HIGH saturation** (the threshold the tactical doc §0 scores at). HIGH does
  NOT mean "descend deeper to prove a null" — the residual in the picked-clean core is thin, so the
  default is **RE-SOURCE to a payable surface (web/API/off-chain/fresh) where the theft that survived
  the audits actually lives.** Stay on the core ONLY for a fresh delta or a genuinely un-swept seam
  (off-chain, new module, cross-chain handoff — Door A is richer there, more adversarial tests to mine).
  Mezo was a textbook fortress (5× audited, Thesis team, DEV-NOTES anticipating their own DoS) → ~15 surfaces
  deep-beast, 2 PoC'd, both honest-dropped, $0. Don't spend immortal depth on a saturated core.
- **N-KILLs = STOP:** N KILL-GATE failures with 0 surviving candidates on a surface → STOP that
  ...
  the signal the core is saturated — log it, RE-SOURCE, and re-engage only on a new version delta.
- **Decision:** emit one of **RE-SOURCE** (saturated core + no delta + no seam → go to a payable surface, do not prove-null here) / **DELTA-ONLY** (audit the
  post-clone git delta + the new modules) / **FULL-DESCENT** (thinner-audited, pre-first-audit,
  off-chain seam, or a fresh adversarial-test surface worth mining). Full saturation-score criteria and
...
**(d) — VERIFY WHAT THE AGENT DIDN'T SAY: check COVERAGE, not just correctness (operator, 2026-06-11: "vérifie aussi ce qu'ils ne sortent pas, vérifie s'ils ne ratent pas des surfaces").** An agent has TWO failure modes and (c) only catches one. (i) What it SAYS is wrong → re-run the claim. (ii) What it DIDN'T say → it swept a NARROWER space than it implies, missed endpoints/backends/files/fields, and its SILENCE reads as "nothing there" when the truth is "not looked at." Mode (ii) is MORE dangerous because a false claim is visible (re-testable) but a MISSED SURFACE is invisible — there is nothing to re-test, just a hole nobody points at. This is exactly how my "BFF has no PII" thesis failed: agents scanned the DECLARED OpenAPI and concluded "none," and the hole (undeclared routes, OTHER backends, non-keyword field names) appeared NOWHERE in their output — a confident null over a sub-space presented as the whole space. So before trusting any agent NULL/"nothing found"/"all X are Y": independently establish the FULL space and confront the agent's coverage against it. Concretely — (1) did the agent enumerate the complete surface, or a convenient subset? (re-derive the full set MYSELF: `find`/`grep` every file, list every host in the bundles, every route incl. undeclared, every backend, every field in the body — not just the ones the agent named). (2) Run the SURFACE-INVENTORY discipline (`~/.claude/skills/SURFACE-INVENTORY-PLAYBOOK.md` — DS-3 CORRECTED: it EXISTS, was a sync gap, now synced; also: enumerate the FULL space yourself, `find`/`grep` every file/host/route/backend/field) / the coverage-ledger above as the anti-(ii) tool: the inventory is the full space; the agent's findings are a coverage claim against it; any surface the agent never names = UNCOVERED, not null. (3) The tell of a coverage hole: an agent NULL whose scope is implicit ("no PII anywhere" — anywhere = where?). Pin the scope: "no PII in WHICH routes/hosts/fields?" — then check whether that scope is the whole space or a slice. A null is only as wide as the space the agent actually enumerated, and I must establish that width myself, not inherit it from the agent's confident tone.

**(e) — A WORKFLOW MAPS COVERAGE; IT NEVER DECIDES. Hand-re-derive EVERY verdict — the REFUTED ones too, not only the REAL — and GREP THE CALLERS before trusting any "never called / unreachable / unused" refutation (Pendle Boros, 2026-06-24).** On Boros, three workflows produced 86+ candidates at ~10:1 false (every agent "REAL/Critical" refuted on hand-read: BOLA cluster ×12 missed the signed-intent auth, margin-insolvency ×3 missed the `timeToMat.max(tThresh)` floor, mark-rate-manip missed the Notional smoothing). (c)/(d) catch the wrong-claims and the missed-surfaces; THIS catches the agents' own REFUTED verdicts — a *wrong* refutation buries a real bug, and a *right* refutation can hide a coverage insight. Mechanical: (1) re-derive each "REFUTED" on the real code, not just each "REAL"; (2) treat every "X is never called / unreachable / unused" as a REACHABILITY claim and `grep -rn 'X('` the callers across the FULL src tree (not the consolidated core) yourself — on Boros the agents refuted 5 orderbook bugs as "`_bookMatch` is never called" and were RIGHT (0 callers), but the insight they missed is that `_bookMatch` (the entire on-chain matcher) is **dead code** → matching is OFF-CHAIN → a whole workflow dissected code that never runs and the LIVE path was elsewhere. A dead-code refutation is a signal to FIND THE LIVE FUNCTION, never a clean close. Budget the hand-verification as the real cost: a workflow that returns N candidates = N hand-reads queued, not N findings. See `feedback_workflow_is_coverage_not_finding_handverify_every_verdict.md`. (Also: on an "is this ECONOMICALLY exploitable?" lead, run the cheapest disconfirmer — magnitude(gain) vs magnitude(cost) from on-chain reads + the deployed fee/rate formulas — BEFORE the expensive fork PoC; on Boros it refuted F-FIDX-STALE-ARB in minutes, 0.31bps funding << trading cost, saving the multi-hour build. `feedback_cheapest_economic_disconfirmer_before_fork_poc.md`.)

**0.5.4 — YOU HAVEN'T FINISHED HUNTING UNTIL EVERY SURFACE IS ATTACKED; "encoded" ≠ "applied"; the operator pushing once = you closed too early.** A whole-target no-find (NULL-COÛTEUX) close is illegal while any in-scope surface lacks an executed theft-attempt artifact — an un-attacked surface is exactly where the theft you're missing lives (read 120/545 lines of a file, never opened the audit PDF that was in git history → not covered). And a lesson sitting in memory does NOT make you obey it — `prove-the-null` was in memory the whole session and you still concluded on a biased PoC. Memory is necessary, not sufficient; these guards live in the SKILL (loaded whole, every invocation) precisely because memory truncates and gets ignored. Re-open velocity is the real metric: if the operator pushing once surfaces something real, you closed too early. On tokenize.it the operator pushed FOUR times and was right four times — that's four fake walls you'd have accepted. Run 0.5.1–0.5.3 on yourself BEFORE reporting any null, so the operator doesn't have to be your disconfirmer.

**0.5.5 — THE DEPLOYED LAYER IS AN IN-SCOPE SURFACE THE CODE-REVIEW NEVER AUDITS — a money-exit the code-review never reads; you have NOT finished hunting it without a read-only deployed pass, and no NULL-COÛTEUX close is legal without one (the August vein, Mezo 2026-06-11).** Proving the CODE sound (even 5×-audited) does NOT clear the deployed state — they are two separate surfaces, and code-review never reads what's actually on-chain. The deployed layer is where August's 39 findings (several Critical) lived: single-key Safe authority (A20/A34), a violated audit-condition (A25), deployed-impl drift. So before any NULL-COÛTEUX close on an audited on-chain target, run the read-only deployed pass (read-only state reads are NOT live testing — allowed even under "no mainnet interaction"; `eth_getStorageAt`/`eth_call`/verified-source-fetch only). Three reads per fund-moving deployed contract: **(1) upgrade authority** — EIP-1967 admin slot → ProxyAdmin → `owner()` → EOA single-key = A20 Critical, or Safe → read threshold/owners (1-of-1 / low-threshold / one key-pair on N Safes = A20/A34). **(2) deployed-impl ≠ repo** — EIP-1967 impl slot → fetch the VERIFIED SOURCE (`/api/v2/smart-contracts/<impl>`) and diff against repo HEAD; a deployed older impl missing a known fix on fund-moving code = theft. **NEVER bytecode-grep for revert strings** (the compiler compresses them → "lacks guard X" is a false positive; Mezo: bytecode-grep said BorrowerOps "lacks all guards", verified source was 1404 lines = repo line-for-line). **(3) deployed oracle/params vs audit conditions** — read the live oracle, DECODE `latestRoundData()` (sane value? right decimals?), compare staleness/MCR/CCR to what the audit ACCEPTED a risk under (A25). The two near-false-positives this pass makes (bytecode-grep "lacks guard", "special-looking oracle") are both killed by reading the ARTIFACT not the structure — same `prove-the-null/shown-body` rule. On Mezo all three were sound (Safe 5-of-9, 6/6 impl=repo, oracle sane=audited) → null, but the null was PROVEN by the reads, not assumed by "the code is a fortress." Full tactical protocol: `firmaudit § DEPLOYED-LAYER READ PASS`.

**0.5.6 — RUN THE COVERAGE LEDGER BEFORE ANY NULL-COÛTEUX/NO-GO CLOSE — enumerate every in-scope file so no un-hunted surface hides the theft primitive you're missing, not just the surfaces with a seam (mandatory, the Mezo 2026-06-11 hole).** Door A/B/C and the seam-thesis aim depth at the high-signal surfaces — a strength — but they share one blind spot: you deep-beast the chosen surfaces, prove them sound, and close the target while N other in-scope files were never enumerated. On Mezo, 8 surfaces were named and proven sound → close, but `find *.sol` showed 20 contracts, 8 NEVER even listed (~1631 LOC), skipped by "no seam" + "canonical-Liquity-dup" — a dup-PRIOR silently became a don't-look. The operator's statistical tell exposes it: if a 279-submission cohort finds things and you find nothing, the likeliest cause isn't "I searched my surfaces badly," it's "I didn't search everywhere and nothing made me confront the gap." So **before any NULL-COÛTEUX/NO-GO close, run `~/.claude/skills/COVERAGE-LEDGER-PLAYBOOK.md`** (DS-3 CORRECTED 2026-06-23: the playbook EXISTS — the earlier 'missing' read was a SYNC GAP, not absence; now synced to ~/.claude/skills/ + sync-skills.sh patched to carry root .md; method also inline below + in DARKSIDE-HUNT.md)**:** mechanically `find` every in-scope file (Solidity + Go + the deployed addresses), classify each `COVERED+artifact` / `COVERED+seam-null` / `SKIPPED+allowed-reason`, and confirm ZERO `UNCOVERED`. "Canonical-dup / helper / trivial / no-seam / small" are NOT standalone skip reasons (they are depth-priors); each such file still earns a delta-pass — open it, grep the project's custom additions (interest layer, native-BTC, recovery-mode, EIP-712 graft), read the untrusted-reachable state-changers, write one artifact or one allowed-skip. This is the completeness twin of 0.5.4 ("you haven't finished hunting until every surface is attacked"): 0.5.4 says no surface lacks an artifact; THIS says you must first ENUMERATE all surfaces so you can't silently omit one. The NULL-COÛTEUX close must be able to say "I enumerated N files; M covered by executed artifact, K skipped for [allowed reasons], 0 uncovered — no un-hunted surface remains." (On Mezo, the ledger-driven sweep of the 8 uncovered contracts returned 0 valid — all canonical-dup or trusted-internal — but the null is now PROVEN complete, not assumed from the 8 chosen surfaces.)
...
**0.5.11 — TEST IN DIRTY NUMBERS; a round-number PoC is a silent null over the whole precision class (Granite #80206, 2026-09-16; playbook v1.6 Gate 3).** A PoC built on clean values (round prices, divisible amounts, whole units) CANNOT structurally reveal the rounding / off-by-one / precision classes — rounding only bites when there is a remainder, so a round number HIDES the class: every test passes, nothing signals, and your "null" is a false-negative over a space you never entered. This is the exact twin of 0.5.7 (verify the state moved) applied to DISCOVERY, not effect. Mandatory on every numeric sink (division, share-conversion, valuation, interest, seizure): drive it with DIRTY inputs — prices carrying cents, non-divisible amounts, prime quantities, decimals != the market's — and show the integer arithmetic (the division, both floors). Round numbers are for the readability of an ALREADY-found bug, never for discovery. The tell: your PoC's prices are $2,000.00 / your amounts divide evenly / your decimals all match. FOIL (2nd Maxim as tool): the dev tests in round dollars → that IS his blind spot → your clean PoC inherits it. *(Granite: A/D/B proven on round prices; bug #80206 beat #92663 on the precision slice — the fix commit itself: "a whole-dollar price divides evenly and hides the defect completely.")*

**0.5.12 — DEDUP PER SINK, NEVER PER CLASS; "the class is known" is a tombstone false-negative at class scale (Granite #99, 2026-09-16; playbook v1.6 Gate 4).** The darkside germe is "the sibling they didn't test" — apply it to your OWN dedup verdict. With N candidates of one class (rounding, zero-share, oracle-staleness…), the tombstone reads the audit/patch at EACH individual sink, N times. Retiring the class in-block ("em01/l01/… = known-audit dups") covers ONE sink and silently claims the N — the same wsts summary-not-primary miss, now at class scale. The free mechanical tell, and it's a GIFT: **a patch or audit that guards only ONE sink of the class is living proof the class is ALIVE at the other sinks** — the dev fixed where he looked; the unguarded siblings are your worklist. FOIL (2nd Maxim as tool): does the dev's patch guard ALL sinks of the class, or one? Read each unguarded sibling in primary source — the class is virgin there. *(Granite: bug #99 was the zero-share class on `borrow`, a sink never read because `deposit`/l01 was judged and the verdict extended by association; the fresh patch guarded only `borrow (>new-debt-shares u0)`, leaving `repay` / `liquidator-paid-shares` / `LP-deposit` open = the class alive at 3 sinks.)*
...
2. **Diff the target against canonical — list every DEVIATION.** Each place this protocol does
   ...
   > **DS-1 fix (darkside-on-darkside 2026-06-23) — the canonical form IS the deviation-completeness ledger.** §0.5.6 enforces a FILE ledger and §0.5.4 a SURFACE ledger, but the DEVIATION space had no completeness gate — and Door C forbids fan-out, so coverage rests on one reader noticing. Close it WITHOUT fan-out (listing the canonical shape is NOT pattern-matching-for-bugs, so it does not suppress the surprise): take the canonical form from step 1, enumerate ITS components as a checklist (NAV source · share lifecycle · fee model · access/ownership · oracle/price path · queue/settlement · upgrade authority · accounting unit), and diff EACH component against the target — confirm ZERO canonical-components left un-diffed before any Door-C null. A deviation you never enumerated is the F-STALE you'll miss; the canonical checklist is what makes "I listed every deviation" provable instead of hoped.
...
|----------|----------|---------|
| **DARKSIDE-HUNT.md** | `~/.claude/skills/darkside/DARKSIDE-HUNT.md` | The tactical doc — full Gate §0 saturation criteria (HIGH → RE-SOURCE, do not prove-null), the 8-step manual loop in full, the 5-axes SC/web/off-chain forms, the negative-results / NO-GO template |
