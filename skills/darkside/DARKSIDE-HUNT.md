...
**Fortress score (Phase 0, before depth):**
- audit_count ≥ 2 → +heavy signal; **audit_count ≥ 3 = HIGH-fortress threshold** (especially if one is an in-repo internal audit)
...
HIGH fortress score → **do NOT run a full darkside descent.** Options:
1. NO-GO immediately and redeploy to the queue, OR
...
3. a **Door-A-only** pass: build the coverage matrix, take the highest-EV N-cell candidate, and
   stop if it gates to null. The matrix is cheap; full descent is not.

The alarm that means STOP: **N KILLs and 0 findings.** That ratio means you are certifying a
fortress, not hunting. Clean corners are certificates of impregnability; they do not pay rent.
