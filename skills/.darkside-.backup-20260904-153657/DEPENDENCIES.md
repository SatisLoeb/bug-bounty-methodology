...
|---|---|---|
| `~/.claude/skills/darkside/DARKSIDE-HUNT.md` | **Primary companion** (internal tactical doc, listed here as the load-bearing reference). Holds the GATE §0 (saturation score / N-KILLs=STOP / depth-is-earned → RE-SOURCE on HIGH saturation), the two-door catalogues (§1 Door A primary / §2 Door B fallback), the 5-axes-as-analyzer §3, the delta-hunt §4, the MANUAL LOOP §5, the THIEF INVENTORY + chain-triage §6, and the EXIT §7 (drop-on-EV ≠ drop-on-fatigue). | Skill has no manual loop, no saturation gate, no exit rule — it is the GATE+DOORS+LOOP+EXIT body. Restore before any engagement. |
...
| `…/memory/feedback_fortress_target_selection_ev_gate.md` | Upstream saturation-score / EV-gate feeding GATE §0 (HIGH saturation = RE-SOURCE to a payable surface, or a fresh-delta-only pass — never dig the saturated core to prove a null). |
