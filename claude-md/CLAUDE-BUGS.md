...
> BYPASS — the payable primitive (theft FIRST, but equally freeze/halt/takeover/insolvency/deanon wherever the
> program pays them). A no-find is the RARE, expensive branch: legal ONLY as a **NULL-COÛTEUX** that
> carries an executed-attack ledger (every payable-impact path — not just every theft-path — killed with an
> executed artifact, then RE-SOURCE to a payable surface), NEVER a comfortable "fortress" you award yourself. The verdict is whatever the executed
...
- **PAYABLE-IMPACT PREFILTER (apply at Phase 0, not at submission — generalized from the money-flow prefilter):** every candidate must produce a PoC line for SOME in-scope payable impact — theft (`loss = $X`) · permanent/temporary freeze · insolvency/bad-debt · governance-takeover by an *untrusted* actor · liveness/halt at this scope's tier (Blockchain/DLT=Crit, app-SC=Med) · deanonymization/PII/disclosure (privacy scopes) — or KILL. A candidate that reaches NO payable class of ANY kind (signature-hygiene / state-misrepresentation with no impact) dies at triage (OKX F-007 killed Week 8/8 despite 24/24). **The KILL condition is "reaches no payable impact", NOT "no `$`":** a freeze/griefing/deanon nets the attacker zero by definition and is still payable — never KILL it for lacking a `loss=$X` line. **The anti-prove-null telos is UNCHANGED** — you still GENERATE a reachable payable finding and do NOT tour clean corners collecting "this is fine"; only the IMPACT UNIT broadened from `$` to the program's full payable set. Instantiate that set for THIS program from `~/.claude/skills/IMPACT-LEDGER-PLAYBOOK.md`. (2026-07-19: the `loss=$X`-only form was declaring fortress-null on scopes that also pay freeze/insolvency/governance/liveness/deanon — Mezo flipped NO-GO→payable once a freeze/griefing pass ran.)
- **PoC = OBSERVED state delta, not counterfactual.** `value_after < value_before` from market state = PROOF; `cost_a − cost_b > 0` from arithmetic = ARGUMENT = dismissed. Triagers dismiss arguments; they cannot dismiss proof. Fork-based mandatory (`vm.createSelectFork`, `deal`, `vm.prank(realAdmin)`, real deployed addresses, ZERO mocks). "Can the PoC produce a PAYABLE impact — steal / freeze / brick / render-insolvent / seize-governance-as-untrusted / deanonymize — using ONLY code that EXISTS?" — no → feature request → KILL.
...
- **Before ANY disconfirmer / prove-null effort, produce two artifacts:** (1) the **impact-exit map** — every place value LEAVES the system (transfer / mint / redeem / withdraw / settle / reward / fee-skim) AND every place liveness/control/confidentiality can BREAK (a queue/settlement that can be frozen, a valset/role/upgrade path an untrusted actor can seize, a path that can emit another party's identity/secret), each with the untrusted actor + primitive that could reach it; (2) **≥3 payable-impact hypotheses spanning MORE than the theft class**, each a one-line `attacker → primitive → observed payable-impact` (theft `loss=$X` | freeze $N | insolvency $N | governance-takeover | halt | deanon of N users). No null-proving until these exist. If you can't name where value leaves OR where liveness/control/privacy breaks, you haven't started the engagement. (Instantiate the class set from `~/.claude/skills/IMPACT-LEDGER-PLAYBOOK.md`.)
...
- **Effort cap (promoted from the darkside lens, quantified):** on a chosen target, **~6 executed attempts PER PAYABLE-IMPACT ROW that all die with 0 survivors → STOP and RE-SOURCE** (the cap is per class — 6 dead theft-attempts do not authorize a NO-GO while the freeze/governance/liveness/deanon rows are un-probed) — do not keep manufacturing disconfirmers to decorate a null. Depth is EARNED where ore remains (fresh <2-audit / off-chain / access-gated), not spent proving an already-picked mine is empty.
...
- **Rule 44 (2026-07-21)** — CLOSED-SOURCE SOLANA = the deployed BPF is the only ground truth; RE + LIVE-FORK it, don't guess from strings → `/solfork` skill (`~/.claude/skills/solfork/SKILL.md`). Pipeline: `solana program dump <PID> prog.so` → recover the on-chain Anchor IDL from its `anchor:idl` PDA (`create_with_seed(find_program_address([],PID)[0],"anchor:idl",PID)`, strip 8+32+4, zlib-decompress — full ix/account/error schema, no anchor CLI) → `llvm-objdump-19 -d --triple=bpfel` → scan `.rodata`/`.data.rel.ro` for embedded pubkey constants (the authorized-caller / oracle / mint a gate compares against) → stand up `solana-test-validator --reset --bpf-program <PID> prog.so` and drive it with solders + raw JSON-RPC. **FIRST-MAXIM gate-pierce (the signature move):** when a body-level check (`InvalidCaller`, `Unauthorized`, a signer/PDA check) blocks a direct call, prove what it demands with EXECUTED artifacts — direct call, arbitrary-CPI forwarder, and a forwarder REDEPLOYED AT the authorized-caller's address (`--bpf-program <AUTH_ADDR> fwd.so`); if it still fires from the authorized address the gate authenticates a forge-proof context (PDA-signer / sysvar introspection) and HOLDS as a NULL-COÛTEUX (record the 3-artifact ledger + the forge-proof requirement, RE-SOURCE) — if it passes, the math/authz behind it is REACHABLE, hunt it (binary-patch the guard's error-immediate branch to study downstream logic; a downstream bug is a real in-scope finding). Bash gotchas: `pkill -f <name>` self-kills the shell (kill by PORT); `simulateTransaction` needs `encoding:base64` or `result` is silently absent (false `err:None`); `spl-token` needs explicit `--fee-payer`; foreground `sleep` blocked (use `until … do sleep`); if the sbf platform-tools tarball is a truncated download, `rm -rf ~/.cache/solana/vX` and rebuild to re-fetch. Proven on Jupiter RFQ v2 (`fd3n`): `InvalidCaller` resisted all three constructions (embeds Jupiter Aggregator v6 as the expected caller). Composes /nuke (routes to it) · /extract · /power (executed on the fork, not derived). Also see `SOLANA-HUNT-CHECKLIST.md`.
