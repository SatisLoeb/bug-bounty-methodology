...
NEXUS uses autonomous subagents for automated validation, security monitoring, and specification generation. These agents prevent errors before they happen rather than fixing them after.

### Available Agents

#### 🗄️ diesel-validator
**Purpose**: Pre-emptive Diesel migration validation
**Invocation**: Automatic via `/diesel-migration` command

**What it does:**
- Validates SQL syntax and rollback logic
- Checks schema consistency (DB vs schema.rs vs Rust models)
- Performs safe dry-run tests on database copy
- Detects missing DEFAULT values on NOT NULL columns
- Identifies column count mismatches

**Prevents**: 80% of migration errors (forgotten `diesel migration run`)

#### 🔒 opsec-auditor
**Purpose**: Continuous OPSEC security monitoring
**Invocation**: Automatic via `/opsec-check` command or pre-commit hook

**What it does:**
- Scans for sensitive data logging (.onion, keys, passwords, IPs)
- Validates RPC URLs are localhost only
- Checks Tor proxy configuration
- Detects hardcoded credentials
- Validates User-Agent strings
- Identifies public port exposure

**Blocks commits**: When critical violations found (95% OPSEC leak prevention)

#### ⚙️ monero-spec-generator
**Purpose**: Automated Monero RPC specification generation
**Invocation**: `/create-monero-spec <function_name>`

**What it does:**
- Researches official Monero RPC documentation
- Analyzes existing codebase patterns
- Generates comprehensive 14-section specifications
- Creates Reality Checks for network functions
- Provides implementation patterns and test structure
- Estimates implementation time

**Time saved**: 30-60 min manual spec writing → 5-10 min automated

### Usage Examples

```bash
# Before applying migration
/diesel-migration  # Invokes diesel-validator agent

# Before committing code
/opsec-check  # Invokes opsec-auditor agent (also runs in pre-commit hook)

# Before implementing Monero RPC function
/create-monero-spec get_balance  # Invokes monero-spec-generator agent
```

### Agent Integration

**Pre-Commit Hooks:**
- `opsec-auditor` blocks commits on critical violations
- `diesel-validator` validates migrations before apply

**Slash Commands:**
- `/diesel-migration` → diesel-validator
- `/opsec-check` → opsec-auditor
- `/create-monero-spec` → monero-spec-generator

**Documentation**: See [.claude/agents/README.md](.claude/agents/README.md) for complete agent documentation

**Performance**: Saves 7-10 hours/week, reduces errors by 70%
