...
rm -rf claude-md methodology skills arsenal
mkdir -p claude-md methodology skills arsenal
...
find claude-md methodology skills arsenal -name ".git" -type d -prune -exec rm -rf {} + 2>/dev/null || true
find claude-md methodology skills arsenal \
  \( -iname "*.key.pem" -o -iname "*.key" -o -iname "*.pem" -o -iname "id_rsa*" \
     -o -iname "*.keystore" -o -iname "*.env" -o -iname "*.p12" \) -delete 2>/dev/null || true
find claude-md methodology skills arsenal -type d \
  \( -name node_modules -o -name __pycache__ -o -name .venv -o -name target \) -prune -exec rm -rf {} + 2>/dev/null || true
find claude-md methodology skills arsenal -type f -size +3M -delete 2>/dev/null || true

# --- 6. HARD secret gate: abort if any real token/key pattern slipped through ---
HITS="$(grep -rIlE 'gh[posu]_[A-Za-z0-9]{36}|sk-[A-Za-z0-9]{20,}|AKIA[0-9A-Z]{16}|-----BEGIN [A-Z ]*PRIVATE KEY-----|xox[baprs]-[A-Za-z0-9-]{10,}' claude-md methodology skills arsenal 2>/dev/null || true)"
if [ -n "$HITS" ]; then
